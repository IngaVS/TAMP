...
namespace path_searching
{
                               ...
                               Eigen::Vector4d end_state,double& model_time,bool use3d)
  {
    ...
    while (!open_set_.empty())
    {
      ...
      for (auto& input:inputs){
        ...
        /* inside map range */
        if (pro_state(0) <= origin_(0) || pro_state(0) >= origin_(0) + map_size_3d_(0) ||
            pro_state(1) <= origin_(1) || pro_state(1) >= origin_(1) + map_size_3d_(1))
        {
  ...
  void KinoAstar::init(ConfigPtr cfg_, ros::NodeHandle nh,bool un)
  {
    ...
    map_size_3d_(0) = cfg_->mapX_range[1] - cfg_->mapX_range[0];
    map_size_3d_(1) = cfg_->mapY_range[1] - cfg_->mapY_range[0];
    origin_(0) = cfg_->mapX_range[0];
    origin_(1) = cfg_->mapY_range[0];
