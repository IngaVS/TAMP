...
namespace map_util{
    ...
    public:
      ...
      void setParam(ConfigPtr config_, ros::NodeHandle& nh){
        ...
        if(Dim == 3){
            ...
            map_size(0) = config_->mapX_range[1] - config_->mapX_range[0] + 1.0e-4;
            map_size(1) = config_->mapY_range[1] - config_->mapY_range[0] + 1.0e-4;
            map_size(2) = config_->mapZ_range[1] - config_->mapZ_range[0] + 1.0e-4;
            origin_d_[0] = config_->mapX_range[0];
            origin_d_[1] = config_->mapY_range[0];
            origin_d_[2] = config_->mapZ_range[0];
        ...
        cloud_buffer_.resize(buffer_size);
        for( int ii = 0; ii < buffer_size; ii++ )
          cloud_buffer_[ii].resize(0);
        point_cloud_sub_ = nh.subscribe("/grid_map_input", 10, &MapUtil::MapBuild, this);
      }
      ...
      void setObs(pcl::PointXYZ pt){
        if(Dim==3){
            ...
            cloud_buffer_[getIndex(index2i)].emplace_back(coord_x,coord_y);
      ...
      ///Get occupied voxels
      std::vector<Eigen::Vector2d> getInputCloud(Eigen::Vector2d pos, double bound) {
        std::vector<Eigen::Vector2d> cloud;
        Veci<Dim> n;
        if(Dim == 3) {
          ROS_ERROR("HoHo! I am lazy~");
        }
        else if (Dim == 2) {
          Eigen::Vector2i id_low, id_high;
          id_low  = floatToInt(pos - Eigen::Vector2d{bound, bound});
          id_high = floatToInt(pos + Eigen::Vector2d{bound, bound});
          boundIndex(id_low);
          boundIndex(id_high);

          for (n(0) = id_low(0); n(0) < id_high(0); n(0)++) {
            for (n(1) = id_low(1); n(1) < id_high(1); n(1)++) {
              if (isOccupied(getIndex(n)))
              {
                int vox_cloud_size = cloud_buffer_[getIndex(n)].size();
                for( int i = 0; i < vox_cloud_size; i++ )
                  cloud.push_back(cloud_buffer_[getIndex(n)][i]);
              }
            }
          }
        }
      ...
      ///Get occupied voxels
      std::vector<Eigen::Vector2d> getInputCloud(Eigen::Vector3d pos, double bound) {
        std::vector<Eigen::Vector2d> cloud;
        Veci<Dim> n;
        if(Dim == 3) {
          ROS_ERROR("HoHo! I am lazy~");
          ...
          Eigen::Vector2i id_low, id_high;
          id_low  = floatToInt(pos.head(2) - Eigen::Vector2d{bound, bound});
          id_high = floatToInt(pos.head(2) + Eigen::Vector2d{bound, bound});
          boundIndex(id_low);
          boundIndex(id_high);
      ...
      ///Get occupied voxels
      std::vector<Eigen::Vector2d> getCloud(Eigen::Vector3d pos, double bound) {
        std::vector<Eigen::Vector2d> cloud;
        Veci<Dim> n;
        if(Dim == 3) {
          ROS_ERROR("HoHo! I am lazy~");
      ...
      vec_Vecf<Dim> getInputCloud() {
        ...
        if(Dim == 3) {
          ...
          for (int idx = 0; idx < (int)cloud_.points.size(); idx++)
          {
            double coord_x = cloud_.points[idx].x;
            double coord_y = cloud_.points[idx].y;
            double coord_z = cloud_.points[idx].z;
            Vec2f pt = Vec2f(coord_x,coord_y);
            cloud.push_back(pt);
          }
        }
    ...
    void publishESDF() {
      ...
      // wxx
      void CheckIfCollisionUsingPosAndYaw(const Eigen::Vector3d &state, bool *res, double flat = 0.1){
        if(Dim==3){
          ...
          for(int i = 0; i < priv_config_->conpts_astar.size(); i++){
            // Eigen::Vector2d rep1 = priv_config_->conpts_astar[i];
            // Eigen::Vector2d rep2 = priv_config_->conpts_astar[(i+1)%priv_config_->conpts_astar.size()];
            // if(rep1[0] > 0){
            //   rep1[0] += res_;
            // }
            // else{
            ...
            Eigen::Vector2d pt = p + rotM * priv_config_->conpts_astar[i];
            Eigen::Vector2d grad;
            // if(flat > 0){
      ...
      ///Map entity
      Tmap map_;
      std::vector<std::vector<Eigen::Vector2d>> cloud_buffer_;
