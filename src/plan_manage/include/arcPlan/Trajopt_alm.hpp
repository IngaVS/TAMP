...
class TrajOpt{
    ...
    public:
        ...
        inline bool setCors(const std::vector<std::vector<Eigen::MatrixXd>> &hPoly_container){
            hPoly_container_ = hPoly_container;
            use_cor_ = true;
            return 1;
        }
            ...
            std::shared_ptr<visualization::Visualization> vis_tool,std::string name = "nn", std::vector<Eigen::VectorXd> ds_container = {})
        {
            ...
            ROS_WARN_STREAM("yawdotmax: "<<yawdotmax);
            ...
            for(int i = 0; i < trajnum; i++) {
                ...
                neqc_num += piecenums[i] * (traj_res+1) * 7; //  vel>0.1 cur  s' >= 0   vel acc  omega
                // neqc_num += piecenums[i] * (traj_res+1) * conpts.size(); //   esdf
                ROS_WARN_STREAM("piecenum: "<<i <<" "<<piecenums[i]);
        ...
        static inline double almCostFunctionCallback(void *func_data, const Eigen::VectorXd &x, Eigen::VectorXd &grad){
            ...
            if( opt->once_optimization )
            {
                opt->iter_wxx++;
                if( opt->iter_wxx % (opt->skip_iter_vis) == 0 )
                {
                    std::cout << "iteration: " << opt->iter_wxx << std::endl;
                    UgvTrajectory  optTraj = opt->getOptTraj();
                    // (opt->vis_tool_)->visualize_se2traj(optTraj, "/visualization/fullshapeTraj_"+opt->name_, opt->conpts);
                    (opt->vis_tool_)->visualize_rectangles(opt->robots_vis, "/visualization/robots_front_end", 1.0, 0.03);
                    std::chrono::milliseconds waitTime((opt->skip_iter_duration_ms));
        ...
        void almGradCost2CT(double &cost,const PolyTrajOpt::MINCO_S3NU<2>& posOpt, const PolyTrajOpt::MINCO_S3NU<1>& tOpt, Eigen::MatrixXd& gdC2c_2d, Eigen::VectorXd& gdC2t_2d, Eigen::MatrixXd& gdC2c_1d, Eigen::VectorXd& gdC2t_1d, int trajid){
            ...
            for (int i = 0; i < N; ++i)
            {
                ...
                for (int j = 0; j <= traj_res; ++j)
                {
                    ...
                    {
                        double yawdot =(vel[0]*acc[1] - acc[0]*vel[1])*ds / (vel.norm() * vel.norm());
                        double cross = (vel[0]*acc[1] - acc[0]*vel[1]);
                        vioYawdot = yawdot*yawdot - yawdotmax*yawdotmax;
                        double swc = scaling_uneqwc_[uneqc_idx];
                        double tmu = mu[uneqc_idx];
                        gx[uneqc_idx] = vioYawdot;
                        if(tmu + rho * swc * vioYawdot > 0){
                            cost += swc * vioYawdot * (tmu + 0.5 * rho * swc * vioYawdot);
                            Eigen::Vector2d gradViolaYDv;
                            Eigen::Vector2d gradViolaYDa;
                            gradViolaYDv = 2.0*yawdot*(Eigen::Vector2d(acc[1], -acc[0])/vel.squaredNorm()-2.0*vel*cross/(vel.squaredNorm()*vel.squaredNorm()))*ds;
                            gradViolaYDa = 2.0*yawdot*(Eigen::Vector2d(-vel[1], vel[0]))*ds/vel.squaredNorm();
                            double gradViolaYDds;
                            gradViolaYDds = 2.0*yawdot*(vel[0]*acc[1] - acc[0]*vel[1])/vel.squaredNorm();

                            gradVel += addAlmGradPro(swc, tmu, vioYawdot, gradViolaYDv);
                            gradAcc += addAlmGradPro(swc, tmu, vioYawdot, gradViolaYDa);
                            gradDs += addAlmGradPro(swc, tmu, vioYawdot, gradViolaYDds);
                        }
                        else{
                            cost += -0.5 * tmu * tmu / rho;
                        }
                    ...
                    if(use_cor_){
                        // std::cout << "11111111111111111111111\n";
                        {
                            ...
                            if( conpts.size() != 4 )
                            {
                                ROS_ERROR("[Trajopt_alm] Error in conpts.size() !!!");
                                ROS_ERROR("[Trajopt_alm] Error in conpts.size() !!!");
                                ROS_ERROR("[Trajopt_alm] Error in conpts.size() !!!");
                            }
                            Eigen::Matrix2Xd robot;
                            robot.resize(2, 4);
                            int robot_v_id = 0;
                            ...
                            for(const Eigen::Vector2d resPt : conpts){
                                ...
                                */
                                ...
                                temp_l_Bl << resPt(0), resPt(1),
                                            -resPt(1), resPt(0);
                                int corr_k = hPoly_container_[trajid][pointid].cols();
                                for(int k = 0; k < corr_k; k++)
                                {
                                    outerNormal = hPoly_container_[trajid][pointid].col(k).head<2>();
                                    double violaPos = outerNormal.dot(absPt - hPoly_container_[trajid][pointid].col(k).tail<2>())+safeMargin;
                                    // std::cout << "outerNormal: "<<outerNormal.transpose()<<"\n";
                                    // std::cout << "hPoly_container_[trajid][pointid].col(k).tail<2>(): "<<hPoly_container_[trajid][pointid].col(k).tail<2>().transpose()<<"\n";
                                    // std::cout << "absPt: "<<absPt.transpose()<<"\n";
                                    // std::cout << "violaPos: "<<violaPos<<"\n";
                                    if(violaPos > 0)
                                    {
                                        double pena, penaD;
                                        positiveSmoothedL1(violaPos, pena, penaD);
                                        cost += esdfWei * pena;
                                        debugEsdf += esdfWei * pena;
                                        gradPos += esdfWei * penaD * outerNormal;
                                        gradVel += esdfWei * penaD * 
                                        (sg * temp_l_Bl / vel.norm() - vel * (rotR * resPt).transpose() / (vel.squaredNorm()) )
                                        * outerNormal;
                                    }
                                }
        ...
        UgvTrajectory getOptTraj(){
            ...
            double maxlat = -1.0;
            for(double t = 0.0; t<= optTraj.getTotalDuration(); t+=0.1){
                double lat = optTraj.getLatAcc(t);
                maxlat = std::max(fabs(lat), maxlat);
            }

            ROS_WARN_STREAM("dptraj max cur: "<<getMaxCur());
            ROS_WARN_STREAM("dptraj max lat: "<<maxlat);
            ROS_WARN_STREAM("dptraj max omega: "<<getMaxOmega());
            return optTraj;
        }
        void debugVis(){
            if(isstart&&isdebug_){
            ...
            vis_tool_->visualize_rectangles(robots_vis, "/visualization/robots_front_end", 1.0, 0.03);
