...
class TrajOpt{
    ...
    public:
        ...
        std::vector<dptraj_mpc::MINCO_S3NU<2>> posOpts;
        std::vector<dptraj_mpc::MINCO_S3NU<1>> tOpts;

        
        inline void init(const ros::NodeHandle &nh_priv){
            ...
            nh_priv.param("isfixGear", isfix_, false);
            nh_priv.param("backW", backW_, 2.0);
        }
        inline int OptimizeSe2Trajectory(
            const Eigen::MatrixXd &headState2d, const Eigen::MatrixXd &tailState2d,
            const double &startVnorm, const double &endVnorm,
            Eigen::VectorXd rdTs_container,
            const std::vector<Eigen::MatrixXd> &inPs_container, const std::vector<Eigen::Vector2d> &gearPos={},
             const std::vector<double> & angles={},  const std::vector<int> singual={1})
        {
        ...
        static inline double almCostFunctionCallback(void *func_data, const Eigen::VectorXd &x, Eigen::VectorXd &grad){
            ...
            for(int trajid = 0; trajid < opt->trajnum; trajid++){
                ...
                if(trajid == 0){
                    IniSt << 0.0, opt->startVnorm_, 0.0;
                }
                else{
                    IniSt << 0.0, 0.0, 0.0;
                }
                if(trajid == opt->trajnum-1){
                    FinSt << Rs_container[trajid].sum(), opt->endVnorm_, 0.0;
                }
                else{
        ...
        void almGradCost2CT(double &cost,const dptraj_mpc::MINCO_S3NU<2>& posOpt, const dptraj_mpc::MINCO_S3NU<1>& tOpt, Eigen::MatrixXd& gdC2c_2d, Eigen::VectorXd& gdC2t_2d, Eigen::MatrixXd& gdC2c_1d, Eigen::VectorXd& gdC2t_1d, int trajid){
            // output gradT gradC 
                ...
                for (int j = 0; j <= traj_res; ++j)
                {
...

                    {
                        ...
                        if(tmu + rho * swc * vioYawdot > 0){
                            ...
                            gradVel += addAlmGradPro(swc, tmu, vioCur, gradViolaYDv);
                            gradAcc += addAlmGradPro(swc, tmu, vioCur, gradViolaYDa);
                            gradDs += addAlmGradPro(swc, tmu, vioVel, gradViolaYDds);
                        }
                        else{
                            cost += -0.5 * tmu * tmu / rho;
                        }
        ...
        std::vector<Eigen::Vector3d> getTraj(double dt){
            ...
            for(int i = 0; i < trajnum; i++){
                dptraj_mpc::PolyTrajectory<2> polytraj = posOpts[i].getTraj();
                dptraj_mpc::PolyTrajectory<1> Ttraj = tOpts[i].getTraj();    
        ...
        dptraj_mpc::UgvTrajectory getOptTraj(){
            dptraj_mpc::UgvTrajectory optTraj;
            for(int i = 0; i < trajnum; i++){
                dptraj_mpc::PolyTrajectory<2> polytraj = posOpts[i].getTraj();
                dptraj_mpc::PolyTrajectory<1> Ttraj = tOpts[i].getTraj();   
